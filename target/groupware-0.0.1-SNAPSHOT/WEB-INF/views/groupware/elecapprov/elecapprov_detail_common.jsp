...
<div id="personnelInfoBody">
	...
	<div>
		<c:if test="${electApprovVO.userId eq userInfo.userId}">
			<a href="/papyrus/elecapprov_edit?elecApprovId=${electApprovVO.id}&categoryId=${electApprovVO.categoryId}&formTableName=${electApprovVO.formTableName}"><button class="btn">수정</button></a>	
		</c:if>
		<a href="/papyrus/elecapprov_list"><button type="button" class="btn">리스트로 돌아가기</button></a>
	</div>
	<div>
		<!-- 결제건이 대기상태이고, 쿼리에서 나온 eamode라는 값이 아직 승인 상태이면 -->
		<c:if test="${electApprovVO.status eq '1' && electApprovVO.eamode eq 'lineuser_pressed_n'}">
			<button type="button" type="button" onclick="allowElecApprov();">승인</button>
			<button type="button" type="button" onclick="disallowElecApprov();">반려</button>
		</c:if>
...
<form id="detail_form" method="post" action="/papyrus/allow_electapprov"></form>
