...
var editEvent = function(event, element, view) {
	...
	for (i = 0; i < user_name_array.length; i++) {
		if (user_name_array[i] != null && user_name_array[i] != "") {
			...
			var obj = {
				value: user_id_array[i],
				name: user_name_array[i],
			};
			array1.push(obj);
		}
	}
	...
	$(document).on("click", ".targetbutton", function() {
		...
		/*		if ($(this.closest("div").attr("id")) == "edit-target-1") {
					console.log("edit-target-1의 버튼입니다.");
					var removetarget = array.indexOf($(this).val());
					array.splice(removetarget, removetarget + 1);
					$(this).remove();
				}
				else if ($(this.closest("div").attr("id")) == "edit-target") {
					console.log("edit-target의 버튼입니다.");
				}*/
	});
