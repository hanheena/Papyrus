...
public class ExcelDownloadService {  
	public void excelDownload(HttpServletResponse response, Map<String, Object> personnel, 
		...
		try {
			...
			workbook =  excelDownloadXSSF(personnel, careerInfo, type,skillInfo);
			System.out.println("#####===excelDownload=========workbook======="+workbook);
			//컨텐츠 타입과 파일명 지정
			response.reset();
	...
	private XSSFWorkbook excelDownloadXSSF(Map<String, Object> personnel, 
			Map<String, Object> careerInfo,String type,List<Map<String, Object>> skillInfo) throws Exception {
		//엑셀 다운로드 시작
		...
		if(type.equals("career")) {
			...
			for(int i = 3; i < 8; i++) {
				...
				if(i ==3) {
					...
					for (int j = 0; j < skillInfo.size(); j++) {
						...
						Date startDate =  format.parse((String) skillInfo.get(j).get("workStartDt"));
						Date endDate = format.parse((String) skillInfo.get(j).get("workEndDt"));
			...
			for (int i = 0; i < skillInfo.size(); i++) {
				...
				for (int j = 0; j < 8; j++) {
					cell = row.createCell(j);
					cell.setCellStyle(styleOfText);
					if(j == 0) cell.setCellValue( skillInfo.get(i).get("workName").toString() );
					if(j == 1) cell.setCellValue( skillInfo.get(i).get("workStartDt") + "\n ~ \n" + skillInfo.get(i).get("workEndDt") );
					if(j == 2) cell.setCellValue( skillInfo.get(i).get("role").toString());
					if(j == 3) cell.setCellValue( skillInfo.get(i).get("workStartDt") + "\n ~ \n" + skillInfo.get(i).get("workEndDt") );
					if(j == 4) cell.setCellValue( skillInfo.get(i).get("workingCompany").toString() );
					if(j == 5) cell.setCellValue( skillInfo.get(i).get("position").toString() );
					if(j == 6) cell.setCellValue( skillInfo.get(i).get("clientName").toString() );
					if(j == 7) cell.setCellValue( skillInfo.get(i).get("devSpecific").toString() );
				}
			}
			...
			for (int i = 0; i < skillInfo.size(); i++) {
				...
				for(int j = 0; j < 11; j ++) {
					...
					if(j == 0) cell.setCellValue( skillInfo.get(i).get("workName").toString() );
					if(j == 1) cell.setCellValue( skillInfo.get(i).get("workStartDt") + "\n ~ \n" + skillInfo.get(i).get("workEndDt") );
					if(j == 2) cell.setCellValue( skillInfo.get(i).get("clientName").toString() );
					if(j == 3) cell.setCellValue( skillInfo.get(i).get("position").toString() );
					if(j == 4) cell.setCellValue( skillInfo.get(i).get("workingCompany").toString() );
					if(j == 5) cell.setCellValue( skillInfo.get(i).get("codeLanguage").toString() );
					...
					if(j == 8) cell.setCellValue( skillInfo.get(i).get("internalSkill").toString() );
					if(j == 9) cell.setCellValue( skillInfo.get(i).get("os").toString() );
					if(j == 10) cell.setCellValue( skillInfo.get(i).get("devTool").toString() );
	...
	//양식다운로드 버튼
	public void downExcel(HttpServletResponse response, Map<String, Object> personnel, 
			Map<String, Object> careerInfo,List<Map<String, Object>> skillInfo ) throws Exception {
				...
				try {
					XSSFWorkbook workbook = null;	//xlsx 확장자를 가진 엑셀 파일로 다운로드
					workbook =  excelDownLoadXSSF(personnel, careerInfo,skillInfo);
					System.out.println("#####===excelDownload=========workbook======="+workbook);
					//컨텐츠 타입과 파일명 지정
					response.reset();
					response.setHeader("Content-Disposition", "attachment; filename=\"" + fileName + ".xlsx\"");
	...
	//양식다운로드 버튼
	@SuppressWarnings("DownLoad")
	private XSSFWorkbook excelDownLoadXSSF(Map<String, Object> personnel, Map<String, Object> careerInfo,
			List<Map<String, Object>> skillInfo) throws Exception {
		...
		//엑셀 다운로드 시작
				...
				for(int i = 3; i < 8; i++) {
					...
					cell.setCellStyle(styleOfText);
					if(i ==3) {									
						cell.setCellValue(  "  년   월    ~    년   월" + " ("+  "    년 "+  "    개월)");
					}
				}
