...
JSGantt.GanttChart =  function(pGanttVar, pDiv, pFormat)
{
   ...
   this.Draw = function()
   {
      ...
      if(vTaskList.length > 0)
      {
         ...
         vLeftTable +=
            '  <TD style="WIDTH: ' + vNameWidth + 'px; HEIGHT: 20px"><NOBR></NOBR>프로젝트</TD>' ;

         if(vShowRes ==1) vLeftTable += '  <TD style="FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 60px; HEIGHT: 20px" align=center nowrap>참여인원</TD>' ;
         if(vShowDur ==1) vLeftTable += '  <TD style="FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 60px; HEIGHT: 20px" align=center nowrap>발주처</TD>' ;
         if(vShowComp==1) vLeftTable += '  <TD style="FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 60px; HEIGHT: 20px" align=center nowrap>완료</TD>' ;
         if(vShowStartDate==1) vLeftTable += '  <TD style="FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 60px; HEIGHT: 20px" align=center nowrap>시작 일</TD>' ;
         if(vShowEndDate==1) vLeftTable += '  <TD style="FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 60px; HEIGHT: 20px" align=center nowrap>종료 일</TD>' ;
            ...
            for(i = 0; i < vTaskList.length; i++)
            {
               ...
               if( vTaskList[i].getGroup()) {
                  if( vTaskList[i].getOpen() == 1) 
                     vLeftTable += '<SPAN id="group_' + vID + '" style="color:#000000; cursor:pointer; font-weight:bold; FONT-SIZE: 12px;">&ndash;</span><span style="color:#000000">&nbsp</SPAN>' ;
                  else
                     vLeftTable += '<SPAN id="group_' + vID + '" style="color:#000000; cursor:pointer; font-weight:bold; FONT-SIZE: 12px;">+</span><span style="color:#000000">&nbsp</SPAN>' ;
               ...
               vLeftTable += 
                  '<span JSGantt.taskLink("' + vTaskList[i].getLink() + '",300,200); style="cursor:default"> ' + vTaskList[i].getName() + '</span></NOBR></TD>' ;
               ...
               if(vShowDur ==1) vLeftTable += '  <TD class=gname style="WIDTH: 60px; HEIGHT: 20px; TEXT-ALIGN: center; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid;" align=center><NOBR>' + projectOrderFrom[i] + '</NOBR></TD>' ;
         ...
         {	
            ...
            {
  		         vRightTable += '<td class=gdatehead align=center style="FONT-SIZE: 12px; HEIGHT: 19px;" width='+vColWidth+'px>'+ vStr + ' 년</td>';
	       ...
	       else if (vFormat=='minute')
	       {
            ...
            if( vTaskList[i].getMile()) {
               ...
               vTaskRight = 1
                  ...
                  '  <div id=taskbar_' + vID + ' title="' + vTaskList[i].getName() + ': ' + vDateRowStr + '" style="height: 16px; width:12px; overflow:hidden; cursor: default;";>';
               ...
               if( vTaskList[i].getGroup()) {
                  ...
                  vRightTable +=
                     '<div id=bardiv_' + vID + ' style="position:absolute; top:5px; left:' + Math.ceil(vTaskLeft * (vDayWidth) + 1) + 'px; height: 7px; width:' + Math.ceil((vTaskRight) * (vDayWidth) - 1) + 'px">' +
                       '<div id=taskbar_' + vID + ' title="' + vTaskList[i].getName() + ': ' + vDateRowStr + '" class=gtask style="background-color:#000000; height: 7px; width:' + Math.ceil((vTaskRight) * (vDayWidth) -1) + 'px;  cursor: defualt;opacity:0.9;">' +
                         '<div style="Z-INDEX: -4; float:left; background-color:#666666; height:3px; overflow: hidden; margin-top:1px; ' +
                               ...
                               'cursor: default;" JSGantt.taskLink("' + vTaskList[i].getLink() + '",300,200);>' +
                           '</div>' +
                        '</div>' +
	            ...
	            vRightTable +=
                     '<div id=bardiv_' + vID + ' style="position:absolute; top:4px; left:' + Math.ceil(vTaskLeft * (vDayWidth) + 1) * 1.24 + 'px; height:18px; width:' + Math.ceil((vTaskRight) * (vDayWidth)) + 'px">' +
                        '<div id=taskbar_' + vID + ' title="' + vTaskList[i].getName() + ': ' + vDateRowStr + '" class=gtask style="background-color:#' + vTaskList[i].getColor() +'; height: 13px; width:' + Math.ceil((vTaskRight) * (vDayWidth) - 1) + 'px; margin-top: 7px; cursor: default;opacity:0.9;" ' +
                           'JSGantt.taskLink("' + vTaskList[i].getLink() + '",300,200); >' +
                           '<div class=gcomplete style="Z-INDEX: -4; float:left; background-color:#262223; height:5px; overflow: auto; margin-top:4px; filter: alpha(opacity=40); opacity:0.4; width:' + vTaskList[i].getCompStr() + '; overflow:hidden">' +
                           '</div>' +
                        '</div>';
                        ...
                        if( g.getCaptionType() ) {
                           ...
                           vRightTable += '<div style="FONT-SIZE:12px; margin-top: 7px; position:absolute; top:-3px; width:120px; left:' + (Math.ceil((vTaskRight) * (vDayWidth) - 1) + 6) + 'px">' + vCaptionStr + '</div>';
	                  }
                  vRightTable += '</div>' ;
               }
            }
...
JSGantt.getMinDate = function getMinDate(pList, pFormat)  
      {
         ...
         if(pList.length == 0){
			return null;	
         	...
         	minDate.setFullYear(pList[0].getStart().getFullYear(), pList[0].getStart().getMonth()-6, pList[0].getStart().getDate());	//최솟값 가정
         	for(i = 0; i <pList.length; i++){
				 vDate.setFullYear(pList[i].getStart().getFullYear(), pList[i].getStart().getMonth()-6, pList[i].getStart().getDate());
				if(Date.parse(minDate) > Date.parse(vDate)){
					minDate = vDate;
				}	
			}//for	
			vDate = minDate; 
         ...
         //vDate.setFullYear(pList[0].getStart().getFullYear(), pList[0].getStart().getMonth()-6, pList[0].getStart().getDate());
   ...
   var maxDate = new Date();
         ...
         if(pList.length == 0){
			return null;	
         	...
         	maxDate.setFullYear(pList[0].getEnd().getFullYear(), pList[0].getEnd().getMonth()+6, pList[0].getEnd().getDate());	//최대값
         	for(i = 0; i <pList.length; i++){
				 vDate.setFullYear(pList[i].getEnd().getFullYear(), pList[i].getEnd().getMonth()+6, pList[i].getEnd().getDate());
				if(Date.parse(maxDate) < Date.parse(vDate)){
					maxDate = vDate;
				}	
			}//for	
			vDate = maxDate; 
         ...
         //vDate.setFullYear(pList[0].getEnd().getFullYear(), pList[0].getEnd().getMonth()+6, pList[0].getEnd().getDate());
